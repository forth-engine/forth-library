...
namespace Forth
{
	struct MeshGen
	{
		...
		static void MakeHypersphere(Buffer4 &input, int subdivision, float radius = 1, const Vector4 &extent = Vector4())
        {
            ...
            if (state.useedge) input.SequenceGrid(state.subdiv * 2 + 6, state.subdiv + 4, state.subdiv + 4);
            else input.SequenceGrid(state.subdiv * 2 + 1, state.subdiv + 1, state.subdiv + 1);
        }
